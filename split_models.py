...
class Exploder(object):
    ...
    def __init__(self, home=home, project=project, app=app, topline=topline,
        ...
        self.imports = dict()
        self.finit = open(self.init, 'w')   # keep open until finished
        ...
        with open(self.bak, 'r') as self.fbak:
            with open(self.sans, 'w') as self.fsans:
                self.fillinit()
        # finished with models.bak, work entirely from models.sans
        self.fsrc = open(self.sans, 'r')
        self.thismodule = None
        # ready to roll out the exploded files
    ...
    def fillinit(self):
        # collect all imports and initlines before splitting out the py files
        i = 0
        self.fbak.seek(0)
        while True:
            ...
            if IMPORT in line:
                ...
                # will need to be imported from models dir so gets a leading dot
    ...
    def writemodelfile(self, classpy=None):
        ...
        if not classpy is None:
            ...
            cleaner.reverse()
            if aggregate:
                cleanest = self.collectfroms(cleaner)
            else:
                cleanest = cleaner
